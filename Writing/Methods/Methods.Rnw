...
\end{equation}
where $\alpha$ is the permissibility parameter. In our experiments, we used $\alpha = 0.1$, that is to say that the threshold is about $10\%$ more permissible and found it to produce high detection rates ($\approx 98\%$). The parameter can be adjusted according to the density of the kernel estimates. Section~\ref{methods:sec2} provides a graphical example through demonstration.
...
Algorithm~\ref{alg2} presents a pseudocode for Phase 2. The input of Phase 2 is $\boldsymbol{X}_\text{P1}$, as our goal is to identify any False Positives in it and delete them. The output is a subset of the input data, denoted by $\boldsymbol{X}_\text{P2} \subseteq \boldsymbol{X}_\text{P1}$, following the removal of the identified False Positives.
...
Our suggestion begins with finding the cluster of the most members associated, denoted by $c_{\text{max}}$ (line 5), and counting the number of elements in that cluster that are from the random sample, $\textit{sample}_\text{N}$ (line 6). This can be done by simply matching the corresponding indices. We denote this number as $n_\text{sample}$. Then we compute two measures. First is the proportion of $n_\text{sample}$ in $c_\text{max}$, given by
\begin{equation}
...
\section{Summary}
\label{methods:sec3}
Both the high- and low-level descriptions of our methodology have been presented in this chapter. We reduced the problem of the underformance by unsupervised anomaly detection tasks into two smaller problems: first of which is to maximise the incidence of True Positives and second of which is to minimise the incidence of False Positives. According to this idea, the overall framework of our method is divided into two phases, each of which focuses on the previously mentioned smaller problems. The following chapter will present the evaluation of our ideas and our method, as well as comparisons with variants of the other existing techniques.
