...
e = numeric()
for (i in 1:nrow(c)) {
    dis = apply(d[1:1850, ], 1, function(x) sqrt((x[1]-c[i,1])^2 + (x[2]-c[i,2])^2))
    ord = order(dis, decreasing = FALSE)[2:10]
    e = c(e, ord[ord > 1750])
}
@

<<phase2-example1, echo=FALSE, fig.align='center', fig.cap="", out.width="0.45\\linewidth", fig.show='hold', cache=TRUE, dependson="phase2-example-data">>=
plot(d, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", asp = 1)
points(d[1:1750, ], col = "grey70")
points(d[c(1851:1855, e), ], col = "grey30", pch = 4)
legend("topleft", "detected instances", pch = 4, adj = c(0, 0.5), cex = 1.3, pt.cex = 1.1)

plot(d, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", asp = 1)
points(d[1851:1855, ], col = "grey30", pch = 4)
points(d[e, ], col = "grey70")
legend("topleft", c("True positive", "False positive"), pch = c(4, 1), adj = c(0, 0.5), cex = 1.3, pt.cex = 1.1)
@

<<phase2-example2, echo=FALSE, fig.align='center', fig.cap="", out.width="0.45\\linewidth", cache=TRUE, dependson="phase2-example-data">>=
plot(d, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", asp = 1)
points(d[1851:1855, ], col = "grey30", pch = 4)
#order(d[e, 1])
...
The second approach is to apply a clustering scheme to clump similar instances into clusters. The resulting clusters can then be compared with the actual normal instances, i.e., the undetected set in phase 1, in an unsupervised fashion. 


Even when the situation is dire such as shown in Figure \ref{fig:phase2-example2}, the data points can still be effectively clustered as shown in Figure \ref{fig:phase2-example3}.

<<phase2-example3, echo=FALSE, fig.align='center', fig.cap="", out.width="0.45\\linewidth", fig.show='hold', cache=TRUE, dependson="phase2-example-data">>=
f = rbind(d[1851:1855, ], d[e[c(16,15,11,10,9,23,31,29)], ])
set.seed(1)
g = kmeans(f, 4, nstart = 20)
set.seed(3)
h = kmeans(a, 8, nstart = 50)

plot(f, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", asp = 1)
points(f[g$cluster==1, ], pch = 3)
points(f[g$cluster==2, ], pch = 4)
points(f[g$cluster==3, ], pch = 2)
...
points(a[h$cluster==1, ], pch = 1, col = "grey90")
points(a[h$cluster==2, ], pch = 1, col = "grey90")
points(a[h$cluster==3, ], pch = 1, col = "grey90")
points(a[h$cluster==4, ], pch = 3, col = "grey70")
points(a[h$cluster==5, ], pch = 0, col = "grey70")
points(a[h$cluster==6, ], pch = 1, col = "grey90")
points(a[h$cluster==7, ], pch = 1, col = "grey90")
points(a[h$cluster==8, ], pch = 4, col = "grey70")
@


\subsection{X-Means Clustering}

k-means and k-medoids are the most efficient and widely accepted clustering schemes:

1. Talk about k-medoids' insensitivity to presence of outliers but every instance in a detected set is a possible outlier under the LOF algorithm.

2. k-means could have been used but x-means is chosen in this case to get rid of having to consider the adequacy of the parameter, $k$. X-means chooses the number of centre that makes the most sense under the Bayesian information criterion.


%----------------------------------------------------------
\subsection{Cluster Identification}

The decision to use distance metrics as a similarity measure is to be consistent with X-Means which is partially based on a distance metric. If the performance of X-Means is reasonable, i.e., purity for each cluster is high enough -> then the distance metric used in the X-Means must qualify as adequately as the value of purity itself.
