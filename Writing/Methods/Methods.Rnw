...
An initial problem to be solved was the high false positive rate 

to identify false positives resulted in 
...
%The goal of Phase 1 is to detect \emph{every} anomalous instance in a data. The way we have decided to achieve this is to run a chosen anomaly detection method in a liberal manner, by setting a more permissive threshold for example, to allow the anomalies that are otherwise less distinguishable from non-anomalous instances to be identified. Because the algorithmic structure of the method is untouched, Phase 1 can be general to any threshold-based anomaly detection methods in theory, which means different methods can be tested, based on domain knowledge and intuitions. A limitation of Phase 1 is that its results contain a higher number of false positives caused by the flexibly defined threshold.
