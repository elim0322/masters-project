...
\begin{equation}
t = \textit{score}_\text{gmax} + (1 - \alpha) \times (\textit{score}_\text{gmax} - \textit{score}_\text{min}),
\end{equation}
...
Figure~\ref{fig:kde2} presents 


\newpage
A random sample of 1,000 instances, whose scores are less than the threshold, is merged with the set of possible anomalies and DBSCAN is performed on the merged set. Figure~\ref{fig:kde-example3} presents a plot of the two previously discussed proportions as a heuristic as well as the system TPR and FPR. There is a relatively large decrease in $p_c$ between the $Eps$ of 0.6 and 0.7, which can also be observed for the TPR. As our heuristic suggestion to select the $Eps$ just before a major decrease in the $p_c$ appears to be the best choice to obtain the most promising TPR and FPR. The line representing $p_r$ can be used to assure that at $Eps=0.6$ \emph{every} instance from the random sample is included in this cluster to further confirm our belief that the cluster indeed consists of non-intrusion instances.
...
<<kde-example3, echo=FALSE, fig.align='center', fig.cap="Results of DBSCAN", out.width="0.68\\linewidth">>=
...
plot(seq(0,1,by=.1), seq(0,1,by=.1), type="n", xlab = "Eps", ylab = "Rate", main = "Various rates across Eps")
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.cluster[4,])
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.sample[4,], lty = 4)
...
legend("right", legend = c(expression('p'[c]), expression("p"[r]), "TPR", "FPR"), col = c("black","black","blue","red"), lty=c(1,4,1,1))
@
