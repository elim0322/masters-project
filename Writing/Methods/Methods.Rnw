...
Our proposed NIDS is built upon the philosophy of \emph{reductionism} and the concept of \emph{min-maxing}. Unsupervised learning tasks are often faced with inevitable underperformance compared to their supervised and semi-supervised counterparts because they are expected to operate with much less information given, eg, missing labels. In order to come up with a robust unsupervised NID scheme capable of producing satisfactory results, our initial approach was to reduce the existing problems, of the underperforming tendency, into simpler, more basic and manageable parts. But the question arises as to how such problems must be defined so that performance is ultimately affected by solving them. To this question, we considered the concept of \emph{min-maxing} to be applicable. More specifically, one way to define the problem of underperformance is as the state of producing an underwhelming number of True Positives (TPs) while also producing an overwhelming number of False Positives (FPs). Then the problem can be reduced and be solved by appropriate methods to \emph{maximise} the incidence of TPs and to \emph{minimise} that of FPs individually. This concept forms the foundation of our NID approach.

For the maximising part, we acknowledge, in general, a positive correlation between True Positive Rate (TPR) and False Positive Rate (FPR), which can be typically observed from a \emph{receiver operating characteristic} (ROC) curve. Using the relationship, our solution is to run the anomaly detection algorithm of our choice in a more permissible, optimistic way which will increase the likelihood of \emph{both} normal and anomalous instances with relatively subtle characteristics to be classified. In this way, we can expect a higher TPR than under ordinary circumstances and the maximisation problem can be easily solved. However, also expected is a higher FPR than usual as more instances, whether they are actually TPs or not, will be classified. We address this problem separately in a separate step which follows the intial classification step. For the \emph{new} problem of the high FPR, we focus solely on identifying which instances correspond to the FPs using a clustering method and remove those instances so that they are not retained in the final result. In other words, those FPs, which we know are surely of normal instances, will no longer be present in the final result in the hope that the remaining instances are only of the anomalous, or intrusion, instances. Then the minimisation problem is essentially solved.

For the rest of this chapter, more technical aspect of our method is presented with a demonstration at the end.
...
The system is divided into two phases whose objectives are to achieve the highest possible TPR and the lowest possible FPR, respectively, using the previously mentioned ideas. Over the course of the research, the final version of our NIDS has seen a few changes from the initial design we had in mind. For example, our intial design for the first phase was to simply achieve a high TPR by setting a more admissible threshold, strictly following the previously mentioned concept. However the final version now involves more steps, which may seem contradictory to and deviating from the concepts and ideas discussed. But the extra steps were implemented as the result of a natural process of adjusting and improving the system, not as an attempt to raise any confusion. Keeping this in mind, the high level system overview is presented in Figure \ref{fig:methods-sys}.
...
\centering
  \begin{tikzpicture}[node distance = 2.3cm, scale=0.8]
    ...
    \node(p2)    [procs, below of=pa, minimum width=8cm]  {Phase 2: \\ Clustering-based FP elimination};
...
<<kde-example2, echo=FALSE, fig.align='center', fig.cap="Result of DBSCAN", out.width="0.68\\linewidth">>=
