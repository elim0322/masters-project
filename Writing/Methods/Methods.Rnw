...
\end{equation}
as it is the most optimal bandwidth when the Gaussian kernel is used. Then the computed kernel density estimates are plotted and the threshold, $t$, is extrapolated from the graph (line 3). To extrapolate, we look for the area of the highest density, check that it is near the score of 1, and extrapolate where the curve ends on the positive side. This will become much more clear in Section \ref{methods:sec2}. Then instances whose LOF scores are greater than or equal to $t$ are subsetted, or labelled, as TFP (True and False Positives) that represent possible anomalies while the remaining instances as TN (True Negatives) that is the normal instances (lines 4--10). Lastly, a random sample is taken from the TN and is appended to the TFP to produce M as the output of Phase 1. Sample size depends on domain knowledge but about $30\%$ of the size of the TFP has been experimentally determined to be appropriate in our case.
...
\State $\text{score} = \Call{LOF}{\mathbf{X}, k}$
\State $\widehat{\text{kde}} = \Call{KDE}{\text{score}, K, h}$
\State Extrapolate $t$
\For{$i=1:n$}
  \If{$\text{score}_i \geq t$}
    \State Subset $\mathbf{X}_i$ as TFP
    \Comment True and False Positives (possible anomalies)
  \Else 
    \State Subset $\mathbf{X}_i$ as TN
    \Comment True Negatives (normal)
  \EndIf
\EndFor
\State Sample from TN
\State $\mathbf{M} \gets$ Merge the sample with TFP
\end{algorithmic}
...
%%% PHASE 2
\subsection{Phase 2}
Let $\mathbf{\epsilon} = \{ 0.1, 0.2, \dotsc, 1 \} \in \mathbb{R}$ 
...
\begin{algorithmic}
\Require $\mathbf{M} \in \mathbb{R}^{\ast \times m}$
