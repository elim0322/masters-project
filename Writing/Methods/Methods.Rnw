...
For the new problem of the high FPR, we focus on identifying which instances correspond to the False Positives using a clustering algorithm. We assume that the majority of the FPs have the homogenous underlying pattern with only few of them to behave differently to the expected pattern. Under the assumption, the False Positives of the expected behaviour are likely to be grouped together using the clustering algorithm of our choice, which can then be removed so that they are not retained in the final result. What must be noted is that the possible anomalies, output from the intial anomaly detection, are instances not conforming to the overall expected behaviour of the given data by definition, which can introduce difficulties in successfully clustering the False Positives. To this problem, we take a sample of normal instances that are not captured by the initial anomaly detection, i.e., True Negatives, and merged them into the set of the possible anomalies to provide further information of the normal pattern. Then the clustering scheme becomes more capable of including the False Positives into the cluster of the True Negatives to effectively produce a ``super" cluster of normal instances that we wish to get rid of. 

The technical aspects of our method is described in Section \ref{methods:sec1} and a demonstration is presented in Section \ref{methods:sec2}.
...
Phase 1 accepts only numerical data as it initially runs the LOF algorithm to score each instance with a degree of being anomalous. Once every instance is scored, the probability distribution of the LOF scores is estimated using the kernel density estimation (KDE), which is then used to plot the estimated probability density function (PDF) to help determine appropriate thresholds. Because we assume that every test dataset contains a higher proportion of normal instances, we expect to observe a very dense curvature whose area under the graph, ie, corresponding probability, is greater than for any other curves and the instances whose scores fall under this region are normal. Then we can extrapolate from the PDF the point at which this major curve ends on the greater side. We lower that point by 10\% to use it as a threshold that is permissible enough to achieve a high TPR. Then those instances whose scores are less than the threshold are labelled as normal while the others are labelled as possible anomalies. Once all the instances are labelled as either normal or possible anomaly, a random sample of reasonably large size, that being a value just under the number of the possible anomalies is taken from the group of the normal instances. The random sample is then merged with the group of the possible anomalies to produce a merged set of instances. This merged set has a property that it contains enough normal instances that will be clustered with the FPs from the possible anomalies using the DBSCAN algorithm in Phase 2. Because we are absolutely sure that more FPs are produced than usual, from the way we set the threshold, we can expect that the merged set consists of higher proportion of normal instances that collectively correspond to the random sample and the FPs.

Phase 2 accepts the merged set as its input and performs the DBSCAN algorithm on it. If one does not have necessary domain knowledge to help decide optimal input parameters for the DBSCAN, the \emph{labels} of the random sample can be used to track down which input parameter should produce the most desirable results. The parameter, $MinPts$, do not have much impact on the result so it can be fixed values while the other parameter, $Eps$, is varied. For each $Eps$, we can measure the proportion of the largest cluster that corresponds to the random sample, denoted by $p_c$. Ideally, we want to observe an increase in this proportion as $Eps$ increases because it will then indicate that the largest clusters are correctly adding FPs incrementally. Another pattern we wish to observe is the $Eps$ value at which the proportion very rapidly drops as this suggests that the parameter is increased over a threshold at which the DBSCAN algorithm starts to include anomalies, which we do not want, in those clusters decreasing their purity. Our heuristic is to select the $Eps$ just before this rapid drop for the most desirable result. Another measure that can provide some extra information is the proportion of the random sample included in the largest cluster, denoted by $p_r$. Ideally, this proportion should be very close to 1 as this will then indicate that most of the random sample is clustered in the largest cluster. Then the instances included in the largest cluster are all removed so that the remaining instances in the result correspond to anomalies only, and thus intrusions.

%%% PHASE 1
\subsection{Phase 1}
Algorithm~\ref{alg1} presents a pseudocode for Phase 1. Let $\boldsymbol{x}_i \in \mathbb{R}^m$ be a vector of traffic attributes, or connection features, for the $i$th connection and $\boldsymbol{X}=\{ x_1, x_2, \dotsc, x_n \} \in \mathbb{R}^{n \times m}$ be the complete matrix of connections, that is the input of Phase 1. The output is a subset of the input data, denoted by $\boldsymbol{X}_\text{P1} \subseteq \boldsymbol{X}$.

First a couple of containers, $\mathit{PA}$ and $N$, are intialised to store the indices of possible anomalies and normal instances, respectively (line 1). Then, the LOF algorithm is run on $\boldsymbol{X}$ with the input parameter $k$ (line 2). The value of $k$ must be greater than the actual number of intrusion instances in $\boldsymbol{X}$, for example if $30\%$ of $\boldsymbol{X}$ is considered to be intrusions then $k = 0.3 \times m$ at least. Once a vector of LOF scores is computed, KDE is run on the scores to estimate their density (line 3). The kernel, $K(u)$, we use is the Gaussian kernel given by
\begin{equation}
...
\end{equation}
as it is the most optimal bandwidth when the Gaussian kernel is used. Then the computed kernel density estimates are plotted and the threshold, $t$, is extrapolated from the graph (line 4). To extrapolate, we look for the area of the highest density, check that it is near the score of 1 (as it corresponds to that of normal instances), and extrapolate where the curve ends on the positive side. This will become much more clear in Section \ref{methods:sec2} through demonstration. Then the indices of instances whose LOF scores are greater than or equal to $t$ are appended to the container $\mathit{PA}$, that represent the possible anomalies, while the others to the container $N$, that represent the normal instances (lines 5--11). It must be noted that $\mathit{PA}$ contains the indices of True Positives and False Positives and $N$ contains those of False Negatives. Then, a random sample, $\mathit{sample}_\text{N}$, is taken from $N$ without replacement, whose elements represent the indices of the False Negatives, and is merged with $\mathit{PA}$ to be stored in $m$ (lines 12--13). Determining the sample size depends on domain knowledge but about $40\%$ of the length of $\mathit{PA}$ has been experimentally determined to be appropriate in our case. The purpose of the random sampling procedure is to provide the future clustering task in Phase 2 with enough information on the pattern of normal instances such that the ``anomalous" and sparse False Positives can be included in the cluster of the normal instances. Because the False Positives are positioned in sparse regions, evidently from running LOF, forming a cluster around them is much more difficult otherwise. The patterns of some False Positives may be significantly different to that of the False Negatives, in which case those False Positives are ignored and remain in the final result as there is simply no other way of determining their true labels under the premise of unsupervised learning. Then we assume the largest cluster, out of all clusters output from the Phase 2 clustering task, to be the cluster of the normal instances, and the members of that cluster are removed. Hence some domain knowledge on the expected number intrusion patterns can help decide the sample size enough for future clustering of normal instances to be the largest in size.

\raggedbottom
Lastly, the instances in $\boldsymbol{X}$ whose indices match with the elements of $m$ are extracted and stored in $\boldsymbol{X}_\text{P1}$ (line 14). Note that the final output, $\boldsymbol{X}_\text{P1}$, is a subset, or a partition, of the original input data, $\boldsymbol{X}$.
...
\State Plot $\widehat{kde}$ and Extrapolate $t$
\For{$i=1:n$}
  \If{$score_i \geq t$}
...
Algorithm~\ref{alg2} presents a pseudocode for Phase 2. The input of Phase 2 is $\boldsymbol{X}_\text{P1}$, as our goal is to identify any False Positives from the input data and exclude them. The output is a subset of the input data, denoted by $\boldsymbol{X}_\text{P2} \subseteq \boldsymbol{X}_\text{P1}$, following the removal of the identified False Positives.

First a container, $C$, is initialised for storing the results of the DBSCAN algorithm (line 1). Then we let $\boldsymbol{\epsilon} = \{ x \in \mathbb{R} \mid 0 < x \leq 1 \}$ be a sequence of values in the range, which is to be used in place of the input parameter, $\epsilon$, of DBSCAN (line 2). The interval for the sequence used in our experiments was 0.1 such that $\boldsymbol{\epsilon} = \{ 0.1, 0.2, \dotsc, 1 \}$. Next, the DBSCAN algorithm is run on the input data, $\boldsymbol{X}_\text{P2}$, for each element in $\boldsymbol{\epsilon}$ (line 4) with a fixed value for the other input parameter, $\mathit{MinPts}$. The output of DBSCAN is a set of clustering index for each instances. In our experiments, we used $\mathit{MinPts}=6$. Varying $\mathit{MinPts}$ does not affect the end result much. If $\mathit{MinPts}$ is changed, the value of $\epsilon$ needs adjusting to get similar results that would be observed for the unchanged $\mathit{MinPts}$. However variations in $\epsilon$ can significantly change the results, for which we suggest a simple heuristic to determine the value of $\epsilon$ for the best result without the need of labels.

Our suggestion begins with finding the cluster of the most members associated, denoted by $c_{\text{max}}$ (line 5) and counting the number of elements in that cluster that are from the random sample, $\textit{sample}_\text{N}$ by matching the corresponding indices (line 6). We denote this number as $n_\text{sample}$. Then we compute two measures. First is the proportion of $n_\text{sample}$ in $c_\text{max}$, given by
\begin{equation}
p_c = \frac{n_\text{sample}}{n(c_\text{max})}.
\end{equation}
We expect to observe an increasing pattern in the values of $p_c$ as we increase $\epsilon$, that is because the size of the boundaries considered to be a cluster increases and an increasing number of the False Negatives is added to $c_\text{max}$. We also expect to observe a drop in $p_c$, which  means $\epsilon$ is becoming large enough to start adding the actual anomalies into $c_\text{max}$ to decrease the proportion. The second measure 


and (2) the proportion of $n_\text{sample}$ in $\textit{sample}_{\text{N}}$
...
\For{Each $\boldsymbol{\epsilon_j}$}
  \State $C_j \gets$ \Call{DBSCAN}{$\boldsymbol{X}_\text{P1}, \boldsymbol{\epsilon_j}, \mathit{MinPts}$}
  \State Determine the largest cluster, $c_{\text{max}}$, in $C_j$
  \State Count the number of instances in $c_{\text{max}}$ that correspond to $\mathit{sample}_\text{N}$, denoted by $n_\text{sample}$
\EndFor
\end{algorithmic}
...
The demonstration is presented using a particular set of instances that seems to accurately portray the sort of results one can expect to obtain. Assuming the LOF algorithm is successfully performed on the data and their respective LOF scores are produced as a result, the kernel density estimates are presented as shown in Figure \ref{fig:kde-example}. The plot shows a major curvature whose probability, of approximately 0.73, is greater than any other curves and therefore must correspond to normal. We draw the threshold, as the dashed line, at a point about 10\% to the left of the first local minimum to maximise classification. A random sample of 1,000 instances, whose scores are less than the threshold, is merged with the set of possible anomalies and DBSCAN is performed on the merged set. Figure~\ref{fig:kde-example2} presents a plot of the two previously discussed proportions as a heuristic as well as the system TPR and FPR. There is a relatively large decrease in $p_c$ between the $Eps$ of 0.6 and 0.7, which can also be observed for the TPR. As our heuristic suggestion to select the $Eps$ just before a major decrease in the $p_c$ appears to be the best choice to obtain the most promising TPR and FPR. The line representing $p_r$ can be used to assure that at $Eps=0.6$ \emph{every} instance from the random sample is included in this cluster to further confirm our belief that the cluster indeed consists of non-intrusion instances.
...
<<kde-example, echo=FALSE, fig.align='center', fig.cap="Density of an arbitrary set of LOF scores", out.width="0.68\\linewidth">>=
...
text(c(2.2, 1.2), "threshold", pos = 4, cex = 1.2)
## probability
# ind = which.min(abs(threshold - d$x))
# sfsmisc::integrate.xy(d$x[1:ind], d$y[1:ind])
@

<<kde-example2, echo=FALSE, fig.align='center', fig.cap="Results of DBSCAN", out.width="0.68\\linewidth">>=
