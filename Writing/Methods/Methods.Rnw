...
This section describes the core concept and schematic overview of the general scheme in Sections \ref{methods:sec1.1} and \ref{methods:sec1.2}, respectively.
...
The core concept of the general scheme is similar to that of \emph{min-maxing}, that is an attempt to achieve a notion of ``best" by maximising highly desirable traits while minimising undesirable ones. The measures that are maximised and minimised are the detection and false positive rates as these are the ultimate performance measures to estimate the capability of an intrusion detection system. Other measures such as time and space complexities, which are useful in evaluating the applicability of a system, are ignored at this stage and are possible candidates for future work.

There are two ideas that contribute towards the core concept of the scheme: (1) the detection and false positive rates, of an unsupervised anomaly detection task, are positively correlated and (2) false positives are expected. These ideas are used in conceptualising the ways in which the the two rates can be respectively maximised and minimised and their in-depth discussion is followed by the rest of the section.


%----------------------------------------------------------
\subsubsection{Positive correlation between detection and false positive rates}
Without going into the algorithmic level of a method and modifying it, the most simplistic approach in achieving a higher detection rate is to set a more permissive, insensitive threshold such that potential anomalies, whose similarity measures have been less distinctive otherwise, could be classified. More false positives will also be included in the result to suggest that the detection and false positive rates are positively correlated when adjusted in this manner. Using this idea, it is possible to set a threshold as liberal as possible to allow every anomaly in a test dataset to be classified, that is essentially equivalent to maximising the detection rate of the given method.

%----------------------------------------------------------
\subsubsection{Expected prevalence of false positives}
For an unsupervised anomaly detection method, the root cause of the problem associated with relatively high incidence of false positives can be a few such as the violation of the fundamental assumption that normal instances are much more frequent than anomalies in a test dataset. In a truly unsupervised fashion, one cannot discover the number of normal, or abnormal, instances in a test dataset, due to the absence of response labels, and must therefore expect a few false positives in the result. In addition to the problem, the previously described maximisation of a detection rate, through the setting of a liberal threshold, is strongly expected to generate a high frequency of false positives in the result. So in any case, false positives are inevitably expected. If the prevalence of the false positives is imminent, it may be advisable to let it happen and find a solution to eliminate them, that is to focus more on resolution than prevention.
...
Figure \ref{fig:methods-scheme} represents the general scheme which forms the basis of our proposed system. The word, \emph{general}, is used as the scheme is closely followed in building the prototypes of the proposed system and is theoretically possible to be generalised for different application domains of unsupervised anomaly detection. An experiment to assess the generalisability of the system is a potential future work.

\begin{figure}[!h]
\centering
  \begin{tikzpicture}[node distance=2cm]
    \node(in)  [simple] {test data};
    \node(p1)  [procs,  below of=in, minimum width=8cm] {Phase 1: \\ liberal unsupervised anomaly detection \\ (maximise detection)};
    \node(pa)  [simple, below of=p1] {all possible anomalies};
    \node(p2)  [procs,  below of=pa, minimum width=8cm] {Phase 2: \\ false positive recognition and elimination \\ (minimise false positives)};
...
The purpose of Phase 1 is to maximise detection by using a liberal threshold in order to allow anomalies, that are otherwise less distinguishable from normal instances, to be classified. This approach can effectively classify anomalies that are embedded in clusters of normal instances and less distinctive. 

Because the algorithmic structure of the method is untouched, Phase 1 can be general to any threshold-based anomaly detection methods. An example is to apply a lower threshold on a set of distances to $k^{th}$ nearest neighbour so that less distant instances, classified as normal otherwise, may be classified as potential anomalies. 


%----------------------------------------------------------
\subsubsection{Phase 2}

The correspondingly high number of false positive is the focus of Phase 2 to be minimised. Recognition of the false positives can be achieved via a clustering method under the assumption that instances for each label are more similar to each other than to those for the other labels. An effective clustering method can be selected for Phase 2 so that one or more clusters of the false positives can be output and removed from the set of possible anomalies. Final output is then expected to consist of \emph{all} the anomalies associated with a test data along with a minimal number of false positives.
...
Using the scheme in the previous section, a two-phase IDS is built as in Figure \ref{fig:methods-sys}. The overall high-level system overview is as follows:
...
The reasoning behind our choice of the density-based methods and the flow of the processes is discussed in the next section.
...
\centering
  \begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
    ...
    \node(4) [procs, below=1cm of 3] {(2) Graph-based \\ threshold \\ determination};
    \node(5) [decis, below=1cm of 4] {(3) Partition};
    \node(6) [io,     left=3cm of 5] {Non-intrusions};
    \node(7) [procs, below=1cm of 6] {(4) Random sampling};
    \node(8) [io,    below=1cm of 7] {Sampled \\ non-intrusions};
    \node(9) [io,    right=3cm of 5] {Possible \\ anomalies};
...
The concept of the \emph{local} approach in finding anomalies within a dataset is first introduced in the LOF algorithm. By measuring the local density, or deviation, of an object with respect to its neighbours, the algorithm is not restricted on the overall shape of the dataset. Whether there exist clusters of arbitrary shapes and varying sizes in the data, LOF can effectively score each instance with the degree of being an outlier with respect to its neighbours.


\subsubsection{(2) Graph-based threshold determination}
Provided that a given dataset satisfies two assumptions which are the majority of the dataset is consisted of non-anomalous instances and anomalies are statistically distinct from the non-anomalous instances, a set of LOF scores produced from the previous process is expected to include a range of highly frequent values. In other words, if 70\% of the dataset corresponds to non-anomalous then approximately 70\% of the computed LOF scores would be 
