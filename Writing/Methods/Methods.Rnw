...
The rest of this chapter is divided into sections that provide high-level overview of our proposed intrusion detection system in Section \ref{methods:sec2}, low-level and phase-specific discussion in Section \ref{methods:sec3}, a comparison to an initial prototype system in Section \ref{methods:sec4} and finally a minimal demonstration of the system in Section \ref{methods:sec5}.
...
This section presents the overview and the low-level discussion of each phase.
...
The input for Phase 1 process is the numeric portion of a test dataset and the output is a set of possible anomalies that include actual anomalies (true positives) and incorrectly classified anomalies (false positives). There are five sub-processes, each of which is described in the rest of this section.
...
%----------------------------------------------------------
\subsubsection{(1) LOF}
The Local Outlier Factor algorithm is run on the input data to score each instance a degree of being an anomaly. As mentioned in Chapter [background], a normal instance will have a score close to one while an anomalous instance will have a score much greater than one.


%----------------------------------------------------------
\subsubsection{(2) Kernel function-based threshold determination}
Provided that a given dataset satisfies two assumptions which are the majority of the dataset consists of normal instances and anomalies are statistically distinct from the normal instances, a set of LOF scores produced from the previous step is expected to include a range of highly frequent values. In other words, if 70\% of the dataset corresponds to normal then approximately 70\% of the computed LOF scores would be close to one.

In order to assess the range of scores that the majority of the instances fall into, kernel density estimation is used to estimate the probability density of the scores and its estimated function is plotted. Then a threshold for the scores is set at the value where the curvature of the largest probability around the score of one ends.


%----------------------------------------------------------
\subsubsection{(3) Partitioning}
Instances whose scores are below the threshold are partitioned as non-intrusions and those whose scores are greater or equal to the threshold are partitioned as possible anomalies. The clean partitioning is possible because all anomalies associated with a test data are classified.


%----------------------------------------------------------
\subsubsection{(4) Random sampling}
A random sample of the non-intrusion instances is taken to avoid introduction of any sampling bias, that is a bias against, or toward, certain instances being more likely to be selected. The sample size, $n$, is by default equal to the number of possible anomalies computed in the previous steps with intention to even out the number of instances belonging to each label for a more efficient clustering task in Phase 2. When the random sample is merged with the set of possible anomalies to produce a new dataset, at least half of it will correspond to normal.


%----------------------------------------------------------
\subsubsection{(5) Merging}
The random sample of non-intrusion instances is merged with the set of possible anomalies. This \emph{increases} the overall size of normal instances in the merged dataset such that a clustering task to be performed in Phase 2 can be more effective at grouping them.
...
The input for Phase 2 process is the merged dataset output from Phase 1 and the output is the intrusion instances defined as anomalies. There are two sub-processes, each of which is described in the rest of this section.
...
\centering
  \begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
    ...
    \node(4) [decis, below=1cm of 3] {(2) Select a cluster \\ based on its size};
...
The clustering algorithm, Density-Based Spatial Clustering of Applications with Noise, is based on the idea of local, or relative, density similar to LOF. The advantage of DBSCAN is that it can effectively cluster data of an arbitrary shape.
