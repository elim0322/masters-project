...
\item Possible anomalies, present in the input dataset, are identified via the density-based anomaly detection algorithm, LOF. Then the dataset is partitioned into two subsets of `normal', i.e., non-intrusion, instances, 3(a), and possible anomalies, i.e., intrusion instances, 3(b). It should be noted that the first subset is purely composed of non-intrusion instances and therefore will be referred to as a Non-Intrusion (NI) set while the second subset consists of true positive and false positive instances as a result of the anomaly detection and will be referred to as a Possibly Anomalous (PA) set.
...
\item The final output, ePA set, contains the intrusion instances associated with the original data with minimal false positive instances.
...
The high level overview of Phase 1 is listed as follows:

\begin{enumerate}
\item The LOF algorithm is applied on the numeric portion of input dataset. A vector of LOF scores are computed as a result, each of which corresponds to each instance.
\item The density of the LOF scores
\item 
\end{enumerate}

The main purpose of Phase 1 is to detect all anomalies associated with a dataset so that the dataset can be flawlessly partitioned into a subset purely composed of `normal' instances and another containing possible anomalies. Hence the target detection rate of LOF, given by the number of anomalous instances detected by the algorithm, i.e., true positive, over the total number of anomalous instances, is one. In order to achieve such a high detection rate, a compromise has been made for the false alarm rate of the detection, denoted by the number of `normal' instances detected as anomalous, i.e., false positive, over the total number of `normal' instances.
