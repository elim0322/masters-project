...
Our suggestion begins with finding the cluster the most members associated, denoted by $c_{\text{max}}$ (line 5), and counting the number of elements in that cluster that are from the random sample, $\textit{sample}_\text{N}$ (line 6). This can be done by simply matching the corresponding indices. We denote this number as $n_\text{sample}$. Then we compute two measures. First is the proportion of $n_\text{sample}$ in $c_\text{max}$, given by
\begin{equation}
