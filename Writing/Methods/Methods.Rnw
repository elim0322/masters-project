...
Our proposed NIDS is built upon the philosophy of \emph{reductionism} and the concept of \emph{min-maxing}. Unsupervised learning tasks are often faced with inevitable underperformance compared to their supervised and semi-supervised counterparts because they are expected to operate with much less information, e.g., missing labels in data. In order to come up with a robust unsupervised anomaly-based intrusion detection framework capable of producing satisfactory results, our initial approach was to reduce the existing problems of underperforming acne into simpler, more basic and manageable parts. But the question arises as to how such problems must be defined so that performance is ultimately affected by solving them. To this question, we considered the concept of \emph{min-maxing} to be applicable. More specifically, one way to define the problem of underperformance is to produce an underwhelming number of True Positives while also producing an overwhelming number of False Positives. Then the problem can be reduced and be approached by appropriate methods to \emph{maximise} the incidence of True Positives and to \emph{minimise} that of False Positives individually. This concept forms the foundation of our intrusion detection approach.

For the maximising part, we acknowledge, in general, a positive correlation between True Positive Rate (TPR) and False Positive Rate (FPR), which can be typically observed from a receiver operating characteristic (ROC) curve. Using the relationship, our solution is to run the anomaly detection algorithm of our choice in a more permissible and optimistic way, which will increase the likelihood of \emph{both} normal and anomalous instances to be captured in the result. In this way, we can expect a higher TPR than under ordinary circumstances, that is our approach to the maximisation problem. However, also expected is a higher FPR than usual as more instances, whether they are actually True Positives or not, will be classified. We address this problem separately in a separate step following the initial optimistic execution of the anomaly detection scheme.

For the new problem of the high FPR, we focus on identifying which instances correspond to the False Positives using a clustering algorithm. We assume that the majority of the False Positives have the homogeneous underlying pattern with a few of them behaving differently from the expected pattern. Under the assumption, the False Positives of the expected behaviour are likely to be grouped together using the clustering algorithm of our choice. Once they are successfully grouped together in a cluster, they can be removed so that they are not retained in the final result. What must be noted is that the possible anomalies, output from the initial anomaly detection, are instances not conforming to the overall expected behaviour of the given data by definition, which can introduce difficulties in successfully clustering the False Positives. To this problem, we take a sample of normal instances that are not captured by the initial anomaly detection, i.e., True Negatives, and merge them with the set of the possible anomalies to provide further information of the normal pattern. Then the clustering scheme becomes more capable of including the False Positives into the cluster of the True Negatives to effectively produce a pure cluster of normal instances.

An overview of our method is described in Section~\ref{methods:sec1}, where a high-level overview is provided with a flow chart, and in-depth algorithmic overviews are presented with pseudocode. In Section~\ref{methods:sec2}, a demonstration is presented to illustrate more intricate details on the execution of our method and Section~\ref{methods:sec3} presents a brief chapter summary.
...
Our proposed method is divided into two phases whose objectives are to achieve the highest possible TPR and the lowest possible FPR, respectively, using the previously mentioned ideas. Over the course of the research, the final version of the method has seen a few changes from the initial design we had in mind. For example, our initial design for the first phase was to simply achieve a high TPR by setting a more admissible threshold, strictly following the previously mentioned concept. However the final version now involves more steps, which may seem contradictory to and deviating from the concepts and ideas discussed. But the extra steps were implemented as the result of a natural process of adjusting and improving the system. Keeping this in mind, high-level overviews of Phases~1 and 2 are presented in Figure~\ref{fig:methods-p1} and \ref{fig:methods-p2}, respectively, to supplement the following descriptions of each phase.
...
\centering
  \begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
    ...
    \node(2) [procs, below=1cm of 1]   {LOF};
    \node(3) [io,    below=1cm of 2]   {Scores};
    \node(4) [procs, below=1cm of 3]   {KDE-based \\ threshold \\ determination};
    \node(5) [decis, below=1cm of 4]   {Partition};
    \node(6) [io,     left=3cm of 5]   {Normal \\ (True Negatives)};
    \node(7) [procs, below=1cm of 6]   {Random sampling \\ with sample size of $n$};
    \node(8) [io,    below=1cm of 7]   {Sampled \\ non-intrusions};
    \node(9) [io,    right=3cm of 5]   {Possible anomalies \\ (True and False Positives)};
    \node(10) [merge] at (8-|5)        {Merge};
    ...
    \draw [arrow] (11) to (12);
  \end{tikzpicture}
  \caption{Flowchart of a high-level Phase 1 overview.}
  \label{fig:methods-p1}
\end{figure}

With reference to Figure~\ref{fig:methods-p1}, Phase 1 accepts numerical data as it initially runs the LOF algorithm to score each instance with a degree of being anomalous. Once every instance is scored, the probability distribution of the LOF scores is estimated using kernel density estimation (KDE), which is then used to plot the estimated probability density function (PDF) to help determine appropriate thresholds. Because we assume that the given test data contains a higher proportion of normal instances, we expect to observe (1) a very dense curvature whose area under the graph, i.e., the probability that the values of the scores fall in that range, is greater than for any other curves and (2) the instances whose scores fall under this region are normal. Then we can extrapolate from the graph, of the PDF, the point at which this major curve ends on the greater side. We lower that point by the value of a parameter to use it as a threshold that is permissible enough to achieve a high TPR. Then those instances whose scores are less than the threshold are labelled as normal (True Negatives) while the others are labelled as possible anomalies (include both True Positives and True Negatives). Once all the instances are labelled as either normal or possible anomaly, a random sample is taken from the set of the normal instances. The random sample is then merged with the set of the possible anomalies to produce a merged set of instances. This merged set has a property that it contains enough of the normal instances that will be clustered with the False Positives from the possible anomalies using the DBSCAN algorithm in Phase 2. Because of the merging, there will be a higher number of normal instances, both False Positives and True Negatives collectively, than that of the actual intrusions in the merged set. Based on this, we identify the largest cluster computed by DBSCAN as the cluster of the normal instances.
...
\centering
  \begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
    ...
    \node(2) [procs, below=1cm of 1] {DBSCAN};
    \node(3) [io,    below=1cm of 2] {Clusters of instances};
    \node(4) [decis, below=1cm of 3] {Cluster size};
  ...
  \end{tikzpicture}
  \caption{Flowchart of a high-level Phase 2 overview}
  \label{fig:methods-p2}
\end{figure}

With reference to Figure~\ref{fig:methods-p2}, Phase 2 accepts the merged set as the input and performs the DBSCAN algorithm on it. If one does not have necessary domain knowledge to help decide what the values of the input parameters for the DBSCAN should be, the \emph{labels} of the random sample can be used to track down which input parameter should produce the most desirable results. The parameter, $\mathit{MinPts}$, do not have much impact on the result so it can be fixed values while the other parameter, $\epsilon$, is varied. For each $\epsilon$, we measure the proportion of the True Negatives (from the sample) residing in the largest cluster, denoted by $p_c$. Ideally, we want to observe an increase in this proportion as $\epsilon$ increases because it will then indicate that the largest cluster is correctly adding the True Negatives incrementally. Another pattern we wish to observe is the $\epsilon$ value at which the proportion starts to decrease rapidly as this suggests that the parameter is increased over a threshold at which the DBSCAN algorithm starts to include anomalies, which we do not want. Our suggestion is to select the $\epsilon$ just before the decrease for the most desirable result. Another measure that can provide some extra information is the proportion of the True Negatives from the random sample included in the largest cluster, denoted by $p_r$. Ideally, this proportion should be very close to 1 as this will then indicate that most of the random sample is successfully clustered in the largest cluster. Then the instances included in the largest cluster are all removed so that the remaining instances in the result correspond to the anomalies only, which are the intrusions.

The rest of the section describes our method in more detail with algorithms and formality.


%%% PHASE 1
\subsection{Phase 1}
Algorithm~\ref{alg1} presents pseudocode for Phase 1. Let $\boldsymbol{x}_i \in \mathbb{R}^m$ be a vector of traffic attributes, or connection features, for the $i$th connection and $\boldsymbol{X}=\{ x_1, x_2, \dotsc, x_n \} \in \mathbb{R}^{n \times m}$ be the complete matrix of connections. The data matrix, $\boldsymbol{X}$, is the input of Phase~1 and the output is a subset of the input data, denoted by $\boldsymbol{X}_\text{P1} \subseteq \boldsymbol{X}$.

\begin{algorithm}
\caption{Phase 1}
\label{alg1}
\begin{algorithmic}[1]
\Require Data matrix, $\boldsymbol{X} \in \mathbb{R}^{n \times m}$
\Ensure Data matrix, $\boldsymbol{X}_\text{P1} \subseteq \boldsymbol{X}$
\State Initialise containers, $\mathit{PA}$ and $N$
\State $score = \Call{LOF}{\boldsymbol{X}, k}$
\State $\widehat{kde} = \Call{KDE}{score, K, h}$
\State Plot $\widehat{kde}$
\Comment to extrapolate threshold
\State Determine $\textit{score}_\text{gmax}$
\State Determine $\textit{score}_\text{min}$
\State Determine $t = \textit{score}_\text{gmax} + (1 - \alpha) \times (\textit{score}_\text{gmax} - \textit{score}_\text{min})$
\For{$i=1:n$}
  \If{$score_i \geq t$}
    \State Append $i$ to $\mathit{PA}$
    \Comment represent possible anomalies
  \Else 
    \State Append $i$ to $N$
    \Comment represent normal
  \EndIf
\EndFor
\State Sample from $N$ without replacement, and Store in $\mathit{sample}_\text{N}$
\State Merge $\mathit{sample}_\text{N}$ and $\mathit{PA}$, and Store in $m$
\State Extract the matching indices of $m$ from $\boldsymbol{X}$, and Store the matched instances as $\boldsymbol{X}_{\text{P1}}$
\end{algorithmic}
\end{algorithm}

Looking at the first line of Algorithm~\ref{alg1}, a couple of containers, $\mathit{PA}$ and $N$, are initialised to store the indices of possible anomalies and normal instances, respectively. Then, the LOF algorithm is run on $\boldsymbol{X}$ with the input parameter $k$ (line 2). The value of $k$ must be greater than the actual number of intrusion instances in $\boldsymbol{X}$, for example if $30\%$ of $\boldsymbol{X}$ is considered to be intrusions then $k = 0.3 \times n,$ where $n$ is the number of intrusions, at least. Once a vector of LOF scores is computed, KDE is run on the scores to estimate their density (line 3). For the kernel, $K(u)$, we use is the Gaussian kernel given by
\begin{equation}
K(u) = \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}u^2},
\end{equation}
where $u =(X_i-x)/h$, as it provides a smooth density curve optimal for graphical extrapolation. For the bandwidth, $h$, we use Silverman's rule of thumb \cite{sil86} given by
\begin{equation}
h = \left( \frac{4\hat{\sigma}^5}{3n} \right)^{\frac{1}{5}} \approx 1.06 \hat{\sigma}n^{-1/5},
\end{equation}
as it is the most optimal bandwidth when the Gaussian kernel is used.

The computed kernel density estimates, $\widehat{\textit{kde}}$, are plotted (line 4) so that the following lines of code can be used to determine the threshold: first, the score at which the global maximum occurs is determined, denoted by $\mathit{score}_\text{gmax}$ (line 5), and second, we determine the score at which the first local minimum, followed by the global maximum, occurs and denote it as $\mathit{score}_\text{min}$ (line 6). Then the threshold, $t$, is computed by the given formula,
\begin{equation}
\label{eq:t}
t = \textit{score}_\text{gmax} - \alpha \times (\textit{score}_\text{gmax} - \textit{score}_\text{min}),
\end{equation}
where $\alpha$ is the permissibility parameter. In our experiments, we used $\alpha = 0.1$, that is to say that the threshold is about $10\%$ more permissible and we found it to produce high detection rates ($\approx 98\%$). The parameter can be adjusted according to the density of the kernel estimates. Section~\ref{methods:sec2} provides a graphical example through demonstration.

Once $t$ is computed, the indices of instances whose LOF scores are greater than or equal to $t$ are appended to the container $\mathit{PA}$, that represent the possible anomalies, while the others to the container $N$, that represent the normal instances (lines 8--14). It must be noted that $\mathit{PA}$ contains the indices of True Positives and False Positives, and $N$ contains those of True Negatives. Then, a random sample, denoted by $\mathit{sample}_\text{N}$, is taken from $N$ without replacement, whose elements represent the indices of the True Negatives, and is merged with $\mathit{PA}$ (lines 15--16). The resulting set of the indices is stored in $m$. Determining the sample size depends on domain knowledge but about $70\%$ of the length of $\mathit{PA}$ has been experimentally determined to be appropriate in our case. The purpose of the random sampling procedure is to provide the future clustering task in Phase 2 with enough information on the pattern of normal instances such that the ``anomalous" and sparse False Positives can be included in the cluster of the normal instances. Because the False Positives are positioned in sparse regions, evidently from having been output from LOF, forming a cluster around them without the added information is much more difficult. The patterns of some False Positives may be significantly different to that of the True Negatives, in which case those False Positives are ignored and remain in the final result as there is simply no other way of determining their true labels under the premise of unsupervised learning. Then we assume the largest cluster, out of all clusters output from the Phase 2 clustering task, to be the cluster of the normal instances, and the members of that cluster are removed. 
%Hence some domain knowledge on the expected number intrusion patterns can help decide the sample size enough for future clustering of normal instances to be the largest in size.

\raggedbottom
Lastly, the instances in $\boldsymbol{X}$ whose indices match with the elements of $m$ are extracted and stored in $\boldsymbol{X}_\text{P1}$ (line 17). Note that the final output, $\boldsymbol{X}_\text{P1}$, is a subset, or a partition, of the original input data, $\boldsymbol{X}$.



%%% PHASE 2
\subsection{Phase 2}
Algorithm~\ref{alg2} presents pseudocode for Phase 2. The input of Phase 2 is $\boldsymbol{X}_\text{P1}$, as our goal is to identify any False Positives in it and delete them. The output is a subset of the input data, denoted by $\boldsymbol{X}_\text{P2} \subseteq \boldsymbol{X}_\text{P1}$, following the removal of the identified False Positives.

The first line of Algorithm~\ref{alg2} starts with a container, $C$, being initialised for storing the results of the DBSCAN algorithm. Then we let $\boldsymbol{\epsilon} = \{ x \in \mathbb{R} \mid 0 < x \leq 1 \}$ be a sequence of values in the range, which is to be used in place of the input parameter, $\epsilon$, of DBSCAN (line 2). The interval for the sequence used in our experiments was 0.1 such that $\boldsymbol{\epsilon} = \{ 0.1, 0.2, \dotsc, 1 \}$. Next, the DBSCAN algorithm is run on the input data, $\boldsymbol{X}_\text{P1}$, for each element in $\boldsymbol{\epsilon}$ (line 4) with a fixed value for the other input parameter, $\mathit{MinPts}$. The output of DBSCAN is a set of \emph{cluster id} for each instances, that is the index of clusters to which each instance is assigned. In our experiments, we used $\mathit{MinPts}=6$. Varying $\mathit{MinPts}$ does not affect the end result much. If $\mathit{MinPts}$ is changed, the value of $\epsilon$ needs adjusting to get similar results that would be observed for the unchanged $\mathit{MinPts}$. However variations in $\epsilon$ can significantly change the results, for which we suggest a simple heuristic to determine the value of $\epsilon$ for the best result without the need of actual labels.

\begin{algorithm}
\caption{Phase 2}
\label{alg2}
\begin{algorithmic}[1]
\Require Data matrix, $\boldsymbol{X}_\text{P1}$
\Ensure Data matrix, $\boldsymbol{X}_\text{P2} \subseteq \boldsymbol{X}_\text{P1}$
\State Initialise a container, $C$
% \State Set $\boldsymbol{\epsilon} = \{ 0.1, 0.2, \dotsc, 1 \}$
\State Set $\boldsymbol{\epsilon} = (0, 1]$
\For{Each $\boldsymbol{\epsilon_j}$}
  \State $C_j \gets$ \Call{DBSCAN}{$\boldsymbol{X}_\text{P1}, \boldsymbol{\epsilon}_j, \mathit{MinPts}$}
  \State Determine $c_{\text{max}}$ in $C_j$
  \State Determine $n_\text{sample}$ in $c_{\text{max}}$
  \State Determine $\boldsymbol{p}_c(j)$
  \State Determine $\boldsymbol{p}_s(j)$
\EndFor
\State Choose the best $\boldsymbol{\epsilon}_j$ from $\boldsymbol{p}_c$ and $\boldsymbol{p}_s$
\State Delete all members of $c_\text{max}$ for the chosen $\boldsymbol{\epsilon}_j$
\State Store the remaining $c_\text{max}$ in $C_j$ as $\boldsymbol{X}_\text{P2}$
\end{algorithmic}
\end{algorithm}

Our suggestion begins with finding the cluster with the most members associated, denoted by $c_{\text{max}}$ (line 5), and counting the number of elements in that cluster that are from the random sample, $\textit{sample}_\text{N}$ (line 6). This can be done by simply matching the corresponding indices. We denote this number as $n_\text{sample}$. Then we compute two measures. First is the proportion of $n_\text{sample}$ in $c_\text{max}$, given by
\begin{equation}
\label{eq:pc}
p_c = \frac{n_\text{sample}}{n(c_\text{max})},
\end{equation}
that is to evaluate the proportion of the largest cluster that corresponds to the True Negatives from the random sample. We expect to observe an increasing pattern in the values of $p_c$ as we increase $\epsilon$, that is because the size of the boundaries considered to be a cluster increases thus increasing the number of True Negatives added to $c_\text{max}$. We also expect to observe a drop in $p_c$, which  means $\epsilon$ is becoming too large to start adding the actual anomalies into $c_\text{max}$ to result in (1) an increase in overall size, thus (2) a decrease in the proportion. The second measure of our suggested heuristic is the proportion of $n_\text{sample}$ in $\textit{sample}_{\text{N}}$, given by
\begin{equation}
\label{eq:ps}
p_s = \frac{n_\text{sample}}{n(\textit{sample}_{\text{N}})},
\end{equation}
to evaluate the proportion of $\textit{sample}_{\text{N}}$ successfully clustered in $c_\text{max}$. We expect to observe a very high value for $p_s$, very close to 1, to ensure that most of $\textit{sample}_{\text{N}}$ is successfully assigned to the largest cluster, $c_\text{sample}$. In summary, we choose the value of $\epsilon$ where just before $p_c$ begins to decrease and confirm that $c_\text{max}$ is indeed the cluster made of the normal instances from a high value of $p_s$. A graphic example will be presented in Section~\ref{methods:sec2}.

The two measures, $p_c$ and $p_s$, are computed for each element in $\boldsymbol{\epsilon}$ (lines 7--8) such that they are vectors with the same length as $\boldsymbol{\epsilon}$. Then the best performing $\boldsymbol{\epsilon}_j$ is chosen using the previously mentioned heuristic (line 10). For the chosen $\boldsymbol{\epsilon}_j$, all the members of the corresponding $c_\text{max}$ are deleted (line 11) so that the remaining instances are the members of the other clusters. These remainders, which we expect to be the intrusion instances, are stored separately as the final output, which we denote as $\boldsymbol{X}_\text{P2}$.
...
The demonstration is presented using a particular random seed of instances that illustrates the previously described method well. It is not necessarily the best performing seed hence the following demonstration is not the representative of the best performing scenario. The results from our experiments were stable, as will be revealed in Chapter 5, to suggest that one can expect to obtain similar results and patterns as in the demonstration.

Assuming the LOF algorithm is successfully performed on the data and their respective LOF scores are produced as a result, the kernel density estimates are presented as shown in Figure~\ref{fig:kde1}. The plot shows a major curvature, around the score of 1, whose probability is given by $\textit{Pr}(0 \leq \textit{score} \leq 2.1) = 0.743$, that is to say $74.3\%$ of the entire data are in this region. The instances whose respective LOF scores fall in this bound are normal because (1) we assumed that the normal instances are more frequent than their intrusion counterparts in the data and (2) the curvature is centred around the score of 1 to indicate these instances are positioned in dense regions, i.e., inliers.

Figure~\ref{fig:kde2} illustrates how the threshold, $t$, can be determined. First the scores at which the global maximum and the local minimum occur, represented by the red and blue dashed lines respectively. This can be done by computing the first derivative of the estimated PDF, i.e., the iterated differences in the PDF, then choosing the value of the score where the signs of the derivatives change (which is the turning point). Then using the formula given by Equation~\ref{eq:t} and the values of the extrema given by the legend in the figure, we compute $t$ as follows:
\begin{align*}
t & = \mathit{score}_\text{gmax} - \alpha \times (\mathit{score}_\text{gmax} - \mathit{score}_\text{gmin}) \\
& = 2.09 - 0.1 \times (2.09 - 1.07) \\
& = 1.99.
\end{align*}
Then we use $t=1.99$ to label all instances whose scores are less than $t$ as normal and the rest as possible anomalies. After that, we take a random sample from the cluster and merge with the possible anomalies, and run the DBSCAN algorithm with $\epsilon = \{0.1, 0.2, \dotsc, 1\}$ and $\textit{MinPts}=6$. Figure~\ref{fig:kde3} presents the plot of $p_c$, $p_s$, TPR and FPR across the values of $\epsilon$. In the figure, there are two plots. Examining the plot on the left, the lines of $p_c$ and $p_s$ are represented by the solid and dashed lines, respectively. The values of $p_c$ appear to start decreasing from $\epsilon=0.3$ however the values of $p_s$ are still increasing to indicate that the decrease is just caused by the False Positives being added to the cluster. At $\epsilon>0.6$, we observe a big decrease in $p_c$, which means that the cluster is adding anomalies. In addition, the value of $p_s$ reaches its maximum value of 1 at $\epsilon=0.6$ to indicate that the entire random sample is assigned to the cluster. Hence, $\epsilon=0.6$ is our choice. On the plot to the right, the actual TPR and FPR are presented by the blue and red lines, respectively. The lines of $p_c$ and $p_s$ are both dotted but the values are the same as in the plot to the left. Checking to see the TPR and FPR corresponding to our choice of $\epsilon$ reveals that the TPR is at $0.97$ while the FPR is at $0.06$.

<<kde1, echo=FALSE, fig.align='center', fig.cap="Estimated density of the LOF scores.", out.width="0.6\\linewidth">>=
load(file = "//Users/eric/masters-project/Writing/Methods/lof_scores.RData")
...
polygon(c(d$x[1], d$x[1:local_min], d$x[local_min]), c(d$y[1], d$y[1:local_min], 0), col = "grey90")
polygon(c(d$x[local_min], d$x[local_min:512], d$x[512]), c(0, d$y[local_min:512], 0), col = "grey60")
legend("topright", c("normal", "possible anomalies"), fill=c("grey90", "grey60"))
text(2, 0.8, expression(paste(Pr, " = ", 0.743)), cex=1.2)
text(3.6, 0.15, expression(paste(Pr, " = ", 0.257)), cex=1.2)
lines(c(1.2, 1.5), c(0.8, 0.8))
lines(c(2.7, 3.1), c(0.15, 0.15))
abline(v=d$x[local_min], lty=2)
text(2.3, -0.09, "2.09", xpd=T)

#text(2, 0.5, expression(paste(Pr, "(", 0, ' \u2264 ', italic(score), ' \u2264 ', 2.1, ")", " = ", 0.743)))
#text(4, 0.2, expression(paste(Pr, "(", 2.1, ' \u003C ', italic(score), ' \u2264 ', 6.2, ")", " = ", 0.257)))

# d$x[local_min]
# sfsmisc::integrate.xy(d$x[1:local_min], d$y[1:local_min])
# sfsmisc::integrate.xy(d$x[local_min:512], d$y[local_min:512])
@

<<kde2, echo=FALSE, fig.align='center', fig.cap="The threshold computed using the global maximum and the local minimum.", out.width="0.6\\linewidth">>=
load(file = "lof_scores.RData")
d = density(lof_30p, bw = "nrd", kernel = "gaussian")
local_min = min(which(diff(sign(diff(d$y))) ==   2) + 1)
local_max = min(which(diff(sign(diff(d$y))) ==  -2) + 1)
t = d$x[local_min]-diff(c(d$x[local_max], d$x[local_min]))*0.1
plot(d, xlab = "LOF scores", main = "Probability distribution of LOF scores")
abline(v=d$x[local_min], lty=2, col="blue")
abline(v=d$x[local_max], lty=2, col="red")
#text(d$x[local_min], 0, expression(paste("local min.", "(", 2.09,")")), pos=4)
#text(d$x[local_max], 0, expression(paste("local min.", "(", 1.07,")")), pos=4)
abline(v = t, lwd=1.5)
#text(c(2.2, 1.2), "threshold", pos = 2, cex = 1.2)
legend("topright", c(expression(paste("global max.", " (", 2.09,")")), expression(paste("local min.", " (", 1.07,")")), expression(paste("threshold", " (", 1.99,")"))), lty=c(2,2,1), col=c("red","blue","black"), lwd=c(1,1,1.5))
...
<<kde3, echo=FALSE, fig.show='hold', fig.align='center', fig.cap="Results of DBSCAN", out.width="0.49\\linewidth">>=
# load("~/../Desktop/masters-project/Data/System_results/dat1_30a.RData")
load("//Users/eric/masters-project/Data/System_results/dat1_30a.RData")
plot(seq(0,1,by=.1), seq(0,1,by=.1), type="n", xlab=expression(epsilon), ylab="Rate", main=expression(paste("p"[c], " and ", "p"[s], " across ", epsilon)))
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.cluster[4,])
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.sample[4,], lty=5)
legend("right", legend = c(expression('p'[c]), expression("p"[s]), expression(paste("chosen ", epsilon))), col = c("black","black","darkgreen"), lty=c(1,5,1), lwd=c(1,1,2))
abline(v=0.6, col="darkgreen", lwd=2)

plot(seq(0,1,by=.1), seq(0,1,by=.1), type="n", xlab = expression(epsilon), ylab = "Rate", main = expression(paste("TPR and FPR across ", epsilon)))
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.cluster[4,], lty=3)
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$p2.proportion.within.sample[4,], lty=3)
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$system.detection.rate[4,], col = "blue")
lines(seq(.1,1,by=.1), system.results_dat1_30a30k$system.false.alarm.rate[4, ], col = "red")
legend("right", legend = c("TPR (0.97)", "FPR (0.06)", expression(paste("chosen ", epsilon))), col = c("blue","red","darkgreen"), lty=c(1,1,1), lwd=c(1,1,2))
abline(v=0.6, col="darkgreen", lwd=2)
@
...
\section{Summary}
\label{methods:sec3}
Both the high- and low-level descriptions of our methodology have been presented in this chapter. We reduced the problem of the underformance by unsupervised anomaly detection tasks into two smaller problems: the first of which is to maximise the incidence of True Positives and the second of which is to minimise the incidence of False Positives. According to this idea, the overall framework of our method is divided into two phases, each of which focuses on the previously mentioned smaller problems. The following chapter will present the evaluation of our ideas and our method, as well as comparisons with variants of the other existing techniques.
...
<<knn-example, echo=FALSE, eval=FALSE, fig.align='center', fig.cap="kNN distance plot for k=5", out.width="0.6\\linewidth", fig.pos="h">>=
load(file = "knn.RData")
