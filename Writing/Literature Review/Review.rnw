...
%A noteworthy contribution to the one class SVM approach in \cite{esk02} is a quarter-sphere SVM approach proposed by Laskov et al.\ \cite{las04} which outperformed the existing one class SVM algorithm.
