...
The error bars on the plots appear to be longer than observed from Figure~\ref{lofResults} to indicate that there is some variation in them. One metric we found to be relevant in evaluating the standard deviations is a measure of dispersion called the coefficient of variation, $c_v$, given by
\[ c_v = \frac{\sigma}{\mu}, \]
